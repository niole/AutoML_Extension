...
import { CircleStackIcon, CheckCircleIcon, DocumentIcon } from '@heroicons/react/24/outline'
...
function Step1DataSource() {
  ...
  const handleSelectDataset = async (dataset: Dataset) => {
    ...
    setLoadingDatasetIds((current: Record<string, boolean>) => ({ ...current, [dataset.id]: true }))
    setDatasetErrorsById((current: Record<string, string>) => {
      const next = { ...current }
    ...
    try {
      ...
      if (detailedDataset) {
        setDatasetDetailsById((current: Record<string, Dataset>) => ({ ...current, [dataset.id]: detailedDataset }))
      }
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Failed to load dataset files'
      setDatasetErrorsById((current: Record<string, string>) => ({ ...current, [dataset.id]: message }))
      addNotification(message, 'error')
    } finally {
      setLoadingDatasetIds((current: Record<string, boolean>) => {
        const next = { ...current }
        delete next[dataset.id]
        return next
      })
  ...
  return (
    <div className="space-y-6">
      <div className="text-sm text-domino-text-secondary">
        ...
        <button
          className="text-domino-accent-purple hover:underline"
          onClick={() => window.open(toDominoTenantUrl(`/u/${userSummary.project_owner}/${userSummary.project_name}/data?tab=datasets`), '_blank')}
        >
