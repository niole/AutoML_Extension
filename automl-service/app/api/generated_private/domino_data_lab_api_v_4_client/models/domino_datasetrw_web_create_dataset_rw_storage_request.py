from __future__ import annotations

from collections.abc import Mapping
from typing import Any, TypeVar, cast

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..types import UNSET, Unset

T = TypeVar("T", bound="DominoDatasetrwWebCreateDatasetRwStorageRequest")


@_attrs_define
class DominoDatasetrwWebCreateDatasetRwStorageRequest:
    """
    Attributes:
        name (str):
        data_plane_id (str):
        pvc_name (str):
        is_data_plane_default (bool | None | Unset):
    """

    name: str
    data_plane_id: str
    pvc_name: str
    is_data_plane_default: bool | None | Unset = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        name = self.name

        data_plane_id = self.data_plane_id

        pvc_name = self.pvc_name

        is_data_plane_default: bool | None | Unset
        if isinstance(self.is_data_plane_default, Unset):
            is_data_plane_default = UNSET
        else:
            is_data_plane_default = self.is_data_plane_default

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update(
            {
                "name": name,
                "dataPlaneId": data_plane_id,
                "pvcName": pvc_name,
            }
        )
        if is_data_plane_default is not UNSET:
            field_dict["isDataPlaneDefault"] = is_data_plane_default

        return field_dict

    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        name = d.pop("name")

        data_plane_id = d.pop("dataPlaneId")

        pvc_name = d.pop("pvcName")

        def _parse_is_data_plane_default(data: object) -> bool | None | Unset:
            if data is None:
                return data
            if isinstance(data, Unset):
                return data
            return cast(bool | None | Unset, data)

        is_data_plane_default = _parse_is_data_plane_default(d.pop("isDataPlaneDefault", UNSET))

        domino_datasetrw_web_create_dataset_rw_storage_request = cls(
            name=name,
            data_plane_id=data_plane_id,
            pvc_name=pvc_name,
            is_data_plane_default=is_data_plane_default,
        )

        domino_datasetrw_web_create_dataset_rw_storage_request.additional_properties = d
        return domino_datasetrw_web_create_dataset_rw_storage_request

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
