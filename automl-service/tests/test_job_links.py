...
class TestBuildDominoJobUrl:
    ...
    def test_returns_none_without_owner(self, monkeypatch):
        _clear_env(monkeypatch, _DOMINO_ENV_KEYS)
        monkeypatch.setenv("DOMINO_USER_HOST", "https://domino.example.com")
        # No DOMINO_PROJECT_OWNER set, and settings cleared
    ...
    def test_returns_none_without_project_name(self, monkeypatch):
        _clear_env(monkeypatch, _DOMINO_ENV_KEYS)
        monkeypatch.setenv("DOMINO_USER_HOST", "https://domino.example.com")
        monkeypatch.setenv("DOMINO_PROJECT_OWNER", "alice")
        import app.config as config_module
        config_module._settings_instance = None
    ...
    def test_full_url_with_owner_and_project(self, monkeypatch):
        _clear_env(monkeypatch, _DOMINO_ENV_KEYS)
        monkeypatch.setenv("DOMINO_USER_HOST", "https://domino.example.com")
        monkeypatch.setenv("DOMINO_PROJECT_OWNER", "alice")
        import app.config as config_module
        config_module._settings_instance = None
        job = _stub_job(domino_job_id="run-xyz", project_name="my-proj")
        result = _build_domino_job_url(job)
    ...
    def test_url_uses_project_name_from_job(self, monkeypatch):
        _clear_env(monkeypatch, _DOMINO_ENV_KEYS)
        monkeypatch.setenv("DOMINO_USER_HOST", "https://domino.example.com")
        monkeypatch.setenv("DOMINO_PROJECT_OWNER", "bob")
        import app.config as config_module
        config_module._settings_instance = None
        job = _stub_job(domino_job_id="run-1", project_name="from-job")
        result = _build_domino_job_url(job)
...
class TestBuildExperimentRunUrl:
    ...
    def test_full_url_with_experiment_id(self, monkeypatch):
        _clear_env(monkeypatch, _DOMINO_ENV_KEYS)
        monkeypatch.setenv("DOMINO_USER_HOST", "https://domino.example.com")
        monkeypatch.setenv("DOMINO_PROJECT_OWNER", "alice")
        import app.config as config_module
        config_module._settings_instance = None
        job = _stub_job(project_name="my-proj")
...
class TestBuildModelRegistryUrl:
    ...
    def test_returns_none_without_owner(self, monkeypatch):
        ...
        import app.config as config_module
        config_module._settings_instance = None
        job = _stub_job(registered_model_name="automlapp-model-1")
        result = _build_model_registry_url(job)
        assert result is None
        config_module._settings_instance = None

    def test_full_url_without_version(self, monkeypatch):
        _clear_env(monkeypatch, _DOMINO_ENV_KEYS)
        monkeypatch.setenv("DOMINO_USER_HOST", "https://domino.example.com")
        monkeypatch.setenv("DOMINO_PROJECT_OWNER", "alice")
        import app.config as config_module
    ...
    def test_full_url_with_version(self, monkeypatch):
        _clear_env(monkeypatch, _DOMINO_ENV_KEYS)
        monkeypatch.setenv("DOMINO_USER_HOST", "https://domino.example.com")
        monkeypatch.setenv("DOMINO_PROJECT_OWNER", "alice")
        import app.config as config_module
...
class TestAttachExternalLinks:
    ...
    def test_sets_all_four_attributes(self, monkeypatch):
        _clear_env(monkeypatch, _DOMINO_ENV_KEYS)
        monkeypatch.setenv("DOMINO_USER_HOST", "https://domino.example.com")
        monkeypatch.setenv("DOMINO_PROJECT_OWNER", "alice")
        import app.config as config_module
